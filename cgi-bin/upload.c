...
{       
	...
	sprintf(buf, "/var/www/cloud/%s", fileNameOnServer);
...
END:       
	...
	sprintf(buf,"sudo mv %s /var/www/cloud/",fileNameOnServer);
	system(buf);
	...
	{
		int size2 = fprintf(fd2, "%d-%d-%d_%d:%d:%d cloud_resources upload_a_file_named:%s\n",(1900+p->tm_year),(1+p->tm_mon), p->tm_mday ,p->tm_hour, p->tm_min, p->tm_sec, fileNameOnServer);
		fclose(fd2);
	}
